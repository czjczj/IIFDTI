...
class TextCNN2(nn.Module):
    ...
    # protein: [bs, protein_len, protein_dim]
    def forward(self, protein): # seq: [batch, seq_len, seq_dim]
        protein = protein.permute([0, 2, 1])
        protein_features = self.convs_protein(protein) #[batch, hid_channels, new_protein_len]
        protein_features = protein_features.permute([0, 2, 1])
        return protein_features #[batch, new_len, hid_dim]
